...
//   const { data: configsResponse, loading: configsLoading, error: configsError } = useApi(
//     api.getConfigs,
//     []  // Run once on mount
//   );

//   // ✅ Setup mutation for updating config
//   const { mutate: updateConfig, loading: updateLoading } = useMutation(
//     (id, companyName, logoUrl, madeInValue, isActive) =>
//       api.updateConfig(id, companyName, logoUrl, madeInValue, isActive)
//   );

//   // ✅ CRITICAL FIX: Initialize config when data loads from API
//   // configsResponse IS the array directly (not response.success)
//   useEffect(() => {
//     if (configsResponse && configsResponse.length > 0) {
//       // ✅ Use first config
//       setConfig(configsResponse[0]);
//     }
//   }, [configsResponse]);
...
//       }
//     } catch (err) {
//       toast.error("Error saving configuration: " + (err.message || "Unknown error"));
...
//         <p>Error loading configuration: {configsError}</p>
//         <p className="text-sm mt-2">Please ensure the backend is running and the database is seeded.</p>
//       </div>
//     );
//   }
...
//     return (
//       <div className="p-6 text-center text-destructive">
//         <p className="font-semibold">No configuration available</p>
//         <p className="text-sm mt-2">Please check if the database has been seeded with initial configuration data.</p>
//         <p className="text-xs mt-2 text-muted-foreground">Run: <code>npm run seed</code> on the backend</p>
//       </div>
//     );
//   }

//   // Render form with database data
//   return (
//     <div>
//       <div className="mb-6">
...
//       <div className="erp-card max-w-2xl p-6 space-y-6">
//         {/* Company Name */}
...
//           />
//           <p className="text-[10px] text-muted-foreground">Database field: company_name</p>
//         </div>
...
//             disabled={updateLoading || isSaving}
//           />
//           <p className="text-[10px] text-muted-foreground">Database field: logo_url</p>
//         </div>
...
//           </Label>
//           <Input
//             value={config.made_in_value || ""}
//             onChange={(e) => setConfig({ ...config, made_in_value: e.target.value })}
//             placeholder="e.g., MADE IN INDIA"
//             disabled={updateLoading || isSaving}
//           />
//           <p className="text-[10px] text-muted-foreground">Database field: made_in_value</p>
//         </div>
...
//         {/* Info Box */}
//         <div className="bg-muted/50 p-3 rounded text-[12px] text-muted-foreground space-y-1">
//           <p className="font-semibold">Configuration Info:</p>
//           <p>ID: <code className="font-mono text-[10px]">{config.config_id?.substring(0, 8)}...</code></p>
...
//         <Button
//           onClick={handleSave}
//           disabled={updateLoading || isSaving || configsLoading}
//           className="w-full"
//         >
//           {updateLoading || isSaving ? "Saving..." : "Save Configuration"}
//         </Button>
//       </div>
//     </div>
//   );
