...
//   const phoneFontSize = isRoll ? "4.5mm" : "4.5mm";
...
//         {/* PRODUCT */}
//         <div style={{ display: "flex", height: dims.rowHeight, borderBottom: "2px solid #000" }}>
//           <div style={{ 
//             width: dims.keyWidth, 
//             backgroundColor: "#000", 
//             color: "white", 
...
//       ? `<img src="${companyLogo}" alt="Logo" style="height: ${logoHeight}; width: 75%; " />` //object-fit: contain;
...
//                     <div class="contact-text">FOR SERVICE </div>
...
function LabelCard({ label, paperType, companyConfig }) {
  ...
  return (
    ...
    >
      ...
      >
        {companyLogo ? (
          <img src={companyLogo} alt="Logo" style={{ height: "100%", width: "70%" }} />
        ) : (
          <div style={{ display: "flex", alignItems: "baseline" }}>
            <span style={{ color: "#2563eb", fontWeight: "bold", fontSize: isRoll ? "5mm" : "4mm" }}>jr</span>
            <span style={{ color: "#374151", fontWeight: "600", fontSize: isRoll ? "5mm" : "4mm" }}>tech</span>
      ...
      <div style={{ display: "flex", flexDirection: "column", paddingLeft: "0.1mm", paddingRight: "0.1mm" }}>
        {[
          { key: "PRODUCT", value: label.product },
          { key: "CAPACITY", value: label.capacity },
          { key: "MODEL NO", value: label.model },
          { key: "SERIAL NO", value: label.serialNo },
          { key: "MFG . CODE", value: label.mfgCode },
          { key: "BATCH NO", value: label.ssn }
        ].map((row, idx) => (
          <div key={idx} style={{ display: "flex", height: dims.rowHeight, borderBottom: "2px solid #FFF" }}>
            <div style={{ 
              width: dims.keyWidth, 
              backgroundColor: "#000", 
              color: "white", 
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <QRCodeSVG value={label.serialNo || "N/A"} size={qrSizePx} level="M" />
        </div>
        ...
        <div style={{ 
          ...
          <div style={{ fontSize: contactFontSize, color: "#374151", fontWeight: "600" }}>FOR SERVICE</div>
...
function PrintPage({ labels, paperType, pageIndex, companyConfig }) {
  ...
  return (
    ...
    >
      {labels.map((label, idx) => (
        <LabelCard key={`page-${pageIndex}-label-${idx}`} label={label} paperType={paperType} companyConfig={companyConfig} />
      ))}
    </div>
  );
}
...
function PrintPreview() {
  ...
  const uniqueSerialIds = useMemo(() => {
    const ids = new Set();
    labelsData.forEach(label => {
      if (label.serialId) ids.add(label.serialId);
    });
    return Array.from(ids);
  }, [labelsData]);
  ...
  const pages = useMemo(() => {
    const config = PAPER_CONFIGS[paperType];
    const result = [];
    for (let i = 0; i < expandedLabels.length; i += config.labelsPerPage) {
      result.push(expandedLabels.slice(i, i + config.labelsPerPage));
    }
    return result;
  }, [expandedLabels, paperType]);
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      if (eventResult.success && eventResult.data) {
        ...
        for (const label of expandedLabels) {
          try {
            await api.createReport(eventId, label.product || '', label.capacity || '', label.model || '', label.serialNo || '', label.mfgCode || '', label.ssn || '', label.serialNo || '');
          } catch (reportError) {
            console.error('Failed to save report:', label.serialNo, reportError);
          }
          ...
          if (label.serialId && !processedSerialIds.has(label.serialId)) {
            processedSerialIds.add(label.serialId);
            try {
              await api.createLabel(label.productId, label.capacityId, label.modelId, label.serialId, label.mfgCode || '', label.ssn || '', label.serialNo || '', null, companyConfig?.config_id || null, label.dupCount || 1);
            } catch (labelError) {
              console.error('Failed to save label:', label.serialNo, labelError);
            }
          }
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    @media print {
      html, body { width: ${config.pageWidth}; height: ${config.pageHeight}; }
...
<body>
      ...
      ${pageLabels.map(label => `
        <div class="label-card">
          ...
          <div class="label-content">
            <div class="label-row"><div class="label-key">PRODUCT</div><div class="label-value">${(label.product || '').toUpperCase()}</div></div>
            <div class="label-row"><div class="label-key">CAPACITY</div><div class="label-value">${(label.capacity || '').toUpperCase()}</div></div>
            <div class="label-row"><div class="label-key">MODEL NO</div><div class="label-value">${(label.model || '').toUpperCase()}</div></div>
            <div class="label-row"><div class="label-key">SERIAL NO</div><div class="label-value">${(label.serialNo || '').toUpperCase()}</div></div>
            <div class="label-row"><div class="label-key">MFG . CODE</div><div class="label-value">${(label.mfgCode || '').toUpperCase()}</div></div>
            <div class="label-row"><div class="label-key">BATCH NO</div><div class="label-value">${(label.ssn || '').toUpperCase()}</div></div>
          </div>
          <div class="made-in">${madeInValue.toUpperCase()}</div>
          <div class="label-footer">
            <div class="qr-section"><img src="https://api.qrserver.com/v1/create-qr-code/?size=${qrSizePx}x${qrSizePx}&data=${encodeURIComponent(label.serialNo || 'N/A')}" alt="QR" /></div>
            <div class="contact-section"><div class="contact-text">FOR SERVICE</div><div class="contact-phone">${callAssistantNo}</div></div>
          </div>
        </div>
      `).join('')}
    </div>
  ...
  const handleCancel = async () => {
    if (uniqueSerialIds.length === 0) { navigate(-1); return; }
    const confirmed = window.confirm(`Are you sure you want to cancel?\n\n${uniqueSerialIds.length} serial number(s) will be voided.`);
    if (!confirmed) return;
    setIsCancelling(true);
    try {
      const result = await api.voidBatchSerials(uniqueSerialIds);
      if (result.success) toast.success(`${result.data.voided} serial number(s) voided`);
      else toast.error('Failed to void serials');
    } catch (error) {
      console.error('Error voiding serials:', error);
      toast.error('Error voiding serials');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      <div className="w-80 bg-white border-r border-gray-200 p-6 flex flex-col">
        ...
        <div className="flex-1">
          <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
            ...
            <div className="mb-4">
              <label className="text-xs font-semibold text-gray-600 mb-2 block">PAPER TYPE</label>
              <div className="relative">
                <button onClick={() => setDropdownOpen(!dropdownOpen)} className="w-full px-3 py-2 text-left text-sm border border-gray-300 rounded-md bg-white flex items-center justify-between hover:border-gray-400 transition-colors" disabled={isPrinting || isCancelling}>
                  <span className="text-gray-700 font-medium">{PAPER_CONFIGS[paperType].name}</span>
                ...
                {dropdownOpen && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-10">
                    {Object.entries(PAPER_CONFIGS).map(([key, config]) => (
                      <button key={key} onClick={() => { setPaperType(key); setDropdownOpen(false); }} className={`w-full px-3 py-2 text-left text-sm hover:bg-gray-50 transition-colors ${paperType === key ? 'bg-primary/10 text-primary font-medium' : 'text-gray-700'}`}>{config.name}</button>
                    ))}
                  </div>
                )}
              </div>
        ...
        <div className="flex gap-3 mt-6">
          <Button variant="outline" onClick={handleCancel} className="flex-1" disabled={isPrinting || isCancelling}>
            {isCancelling ? (<><Loader2 size={16} className="mr-2 animate-spin" /> VOIDING...</>) : 'CANCEL'}
          </Button>
          <Button onClick={handlePrint} className="flex-1 bg-primary hover:bg-primary/90" disabled={isPrinting || isCancelling}>
            {isPrinting ? (<><Loader2 size={16} className="mr-2 animate-spin" /> SAVING...</>) : (<><Printer size={16} className="mr-2" />PRINT NOW</>)}
          </Button>
        </div>
      </div>

      <div className="flex-1 p-6 overflow-auto bg-gray-200">
        <h2 className="text-center text-sm font-bold text-gray-600 mb-4">{PAPER_CONFIGS[paperType].name} PREVIEW</h2>
        <div className="flex flex-col items-center" style={{ transform: "scale(0.6)", transformOrigin: "top center" }}>
          {pages.map((pageLabels, pageIdx) => (<PrintPage key={pageIdx} labels={pageLabels} paperType={paperType} pageIndex={pageIdx} companyConfig={companyConfig} />))}
        </div>
      </div>
    </div>
  );
