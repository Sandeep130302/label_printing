...
//                             parseInt(label.duplicate_count) > 1 
//                             ? 'bg-primary/10 text-primary' 
//                             : 'bg-muted text-muted-foreground'
//                           }`}>
//                             {label.duplicate_count || 1}
//                           </span>
//                           </td>
...
// ============================================
// PAPER CONFIGURATIONS - EXACT MATCH WITH PrintPreview.jsx
// ============================================
const PAPER_CONFIGS = {
  A4_SHEET: {
    name: "A4 SHEET",
...
// ============================================
// LABEL DIMENSIONS - EXACT MATCH WITH PrintPreview.jsx
// ============================================
const LABEL_DIMENSIONS = {
  A4_A3: {
    headerHeight: "9mm",
...
function Reports() {
  ...
  const openReprintWindow = (label, copies, paperType) => {
    ...
    // ============================================
    // PRINT TEMPLATE - EXACT MATCH WITH PrintPreview.jsx
    // ============================================
...
<body>
      ...
      ${pageLabels.map(lbl => `
        <div class="label-card">
          ...
          <div class="label-content">
            <div class="label-row"><div class="label-key">PRODUCT</div><div class="label-value">${(lbl.product_name || '').toUpperCase()}</div></div>
            <div class="label-row"><div class="label-key">CAPACITY</div><div class="label-value">${(lbl.capacity_name || '').toUpperCase()}</div></div>
            <div class="label-row"><div class="label-key">MODEL NO</div><div class="label-value">${(lbl.model_name || '').toUpperCase()}</div></div>
            <div class="label-row"><div class="label-key">SERIAL NO</div><div class="label-value">${(lbl.serial_number || '').toUpperCase()}</div></div>
            <div class="label-row"><div class="label-key">MFG . CODE</div><div class="label-value">${(lbl.manufacturing_code || '').toUpperCase()}</div></div>
            <div class="label-row"><div class="label-key">BATCH NO</div><div class="label-value">${(lbl.ssn || '').toUpperCase()}</div></div>
          </div>
          <div class="made-in">${madeInValue.toUpperCase()}</div>
          <div class="label-footer">
            <div class="qr-section"><img src="https://api.qrserver.com/v1/create-qr-code/?size=${qrSizePx}x${qrSizePx}&data=${encodeURIComponent(lbl.serial_number || 'N/A')}" alt="QR" /></div>
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <p className="text-sm font-medium text-primary tracking-wide">HISTORY</p>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6 bg-card border border-border rounded-lg p-4">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 border border-border rounded-md text-sm font-medium text-foreground hover:bg-muted transition-colors min-w-[140px]"
          >
            <Calendar size={16} className="text-muted-foreground" />
            {TIME_FILTERS.find(f => f.key === timeFilter)?.label}
            <svg className={`w-4 h-4 ml-auto transition-transform ${timeDropdownOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          ...
          {timeDropdownOpen && (
            <>
              ...
              <div className="absolute left-0 mt-2 w-40 bg-background border border-border rounded-md shadow-lg py-1 z-50">
                {TIME_FILTERS.map((option) => (
                  <button
                    key={option.key}
                    onClick={() => handleTimeFilterChange(option.key)}
        ...
        <div className="relative">
          ...
          >
            ...
            {currentFieldConfig?.label || "Filter By..."}
            <svg className={`w-4 h-4 ml-auto transition-transform ${fieldDropdownOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
            </svg>
          </button>
            ...
            <>
              ...
              <div className="absolute left-0 mt-2 w-40 bg-background border border-border rounded-md shadow-lg py-1 z-50">
                {FIELD_FILTERS.map((option) => (
                  <button
                    key={option.key || 'none'}
                    onClick={() => handleFieldFilterChange(option.key)}
                    className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${
                      fieldFilter === option.key 
                        ? 'bg-primary/10 text-primary font-medium' 
                        : 'text-foreground hover:bg-muted'
                    }`}
                  >
        ...
        {/* Search Input - Only show when a field is selected */}
        {fieldFilter && (
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        {eventsLoading ? (
          <div className="p-8 text-center">
            <RefreshCw className="animate-spin mx-auto mb-2 text-muted-foreground" size={24} />
          ...
          <table className="w-full">
            <thead className="bg-muted/50 border-b border-border">
              <tr>
                <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Event Number</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Date & Time</th>
                <th className="text-center px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Labels</th>
                <th className="text-center px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {events.map((event) => (
                <tr key={event.overall_report_id} className="hover:bg-muted/30 transition-colors">
                  <td className="px-4 py-3">
                    <span className="font-mono text-sm font-medium text-foreground">{event.event_number}</span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary/10 text-primary">
                      {event.number_of_label_printed || event.unique_labels || 0}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <Button
      ...
      <Dialog open={!!detailEvent} onOpenChange={() => setDetailEvent(null)}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Printer size={18} />
              Event Details: {detailEvent?.event_number}
            </DialogTitle>
          </DialogHeader>
              ...
              {eventLabels.length > 0 ? (
                <>
                  <div className="text-sm text-muted-foreground mb-3">
                    {eventLabels.length} unique label(s)
                    {activeSearch.field && activeSearch.value && (
                      <span> matching "{activeSearch.value}"</span>
                    )}
                  </div>
                  <table className="w-full text-sm">
                    <thead className="bg-muted/50 sticky top-0">
                      <tr>
                        ...
                        <th className="text-left px-3 py-2 font-semibold">MFG Code</th>
                        <th className="text-left px-3 py-2 font-semibold">SSN</th>
                        <th className="text-center px-3 py-2 font-semibold">Reprint</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border">
                      {eventLabels.map((label, idx) => (
                        <tr key={idx} className="hover:bg-muted/30">
                          ...
                          <td className="px-3 py-2">{label.capacity_name}</td>
                          <td className="px-3 py-2">{label.model_name}</td>
                          <td className="px-3 py-2 font-mono font-medium text-primary">{label.serial_number}</td>
                          <td className="px-3 py-2">{label.manufacturing_code || '—'}</td>
                          <td className="px-3 py-2">{label.ssn || '—'}</td>
                          <td className="px-3 py-2 text-center">
                            <Button
