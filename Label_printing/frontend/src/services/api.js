...
// // // ============================================
// // // ✅ NEW: Get UNIQUE reports by event
// // // Returns deduplicated serial numbers with label_count
// // // Use this for the event details popup
// // // ============================================
...
// Get all dashboard data (stats + events + chart)
export async function getDashboardData(filter = 'thisMonth') {
  return apiCall(`/dashboard?filter=${filter}`);
}
...
export async function getRecentEvents(limit = 5) {
  return apiCall(`/dashboard/events?limit=${limit}`);
}

// Get chart data with filter
export async function getDashboardChart(filter = 'thisMonth') {
  return apiCall(`/dashboard/chart?filter=${filter}`);
}
