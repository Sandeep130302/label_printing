...
-- STEP 1
CREATE TEMP TABLE report_cleanup AS
SELECT DISTINCT ON (COALESCE(overall_report_id, '00000000-0000-0000-0000-000000000000'), serial_number)
  report_id as keep_id,
  overall_report_id,
  serial_number,
  COUNT(*) OVER (
    PARTITION BY COALESCE(overall_report_id, '00000000-0000-0000-0000-000000000000'), serial_number
  ) as total_prints,
  created_at as first_print,
  MAX(CASE WHEN is_reprinted THEN 1 ELSE 0 END) 
    OVER (
      PARTITION BY COALESCE(overall_report_id, '00000000-0000-0000-0000-000000000000'), serial_number
    )::boolean as any_reprinted
FROM report
WHERE serial_number IS NOT NULL
  AND serial_number != ''
ORDER BY 
  COALESCE(overall_report_id, '00000000-0000-0000-0000-000000000000'),
  serial_number, 
  created_at ASC;
...
WHERE report_id IN (
  ...
  FROM report r
  INNER JOIN report_cleanup rc 
    ON COALESCE(r.overall_report_id, '00000000-0000-0000-0000-000000000000') =
       COALESCE(rc.overall_report_id, '00000000-0000-0000-0000-000000000000')
    AND r.serial_number = rc.serial_number
  WHERE r.report_id != rc.keep_id
);
